...
I got close to the 100% of laptop functionality working on Arch Linux, so you can use it as a signal when buying a new laptop.
...
If you are interested in installation only you can skip to [[*Base installation of Arch Linux][Base installation of Arch Linux]].
...
You may not like systemd, but it is winning.
...
When I used other Linux distributions with big scheduled releases I was much more frequently hit by old version of packages rather than getting something too bleading edge on arch.
...
[[https://wiki.archlinux.org/index.php/HiDPI][See Arch Wiki HiDPI section]].
...
It seems that HiDPI is correctly handled system wide (xdpyinfo reports correct resolution), but just some old apps did not adapt yet.
