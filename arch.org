...
It should be mostly similar on modern laptops coming with GPT/UEFI.
...
* Base installation
Mostly steps from https://wiki.archlinux.org/index.php/beginners'_guide.
...
My =/boot/loader/loader.conf= now looks like:
#+BEGIN_EXAMPLE
timeout 4
default arch
editor 0
...
My =/boot/loader/entries/= directory only contains arch.conf.
** Network
...
#+END_SRC
** libgl
When =pacman -S emacs= I had to choose libgl implementation.
There were 4 alternative libgl implementations. I went for mesa-libgl, since alternatives were from nvidia and this thinkpad has an integrated intel graphics card.
** x86-input
I had a choice between evdev and libinput.
