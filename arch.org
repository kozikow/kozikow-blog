...
#+BEGIN_SRC bash :results output
  ...
  startx
#+END_SRC
*** gnome-terminal hangs
gnome-terminal worked initially, but at some time it refused to start.
In case you run into the same problem I strongly recommend [[https://wiki.archlinux.org/index.php/Termite][termite]] as an alternative.
