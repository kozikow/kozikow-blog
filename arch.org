...
** Auto starting
- For init scripts that apply system wide you need to create a [[https://wiki.archlinux.org/index.php/systemd][systemd service]] (Type=oneshot for just init scripts)
...
#+BEGIN_SRC bash :results output
  ...
  cp /usr/share/applications/emacs.desktop ~/.config/autostart
#+END_SRC
** Screen lock
