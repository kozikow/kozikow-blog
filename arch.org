...
#+BEGIN_SRC bash :results output
  ...
  hwclock --systohc --utc
#+END_SRC
I must admit that I had some issues with GB locale and I reverted to US locale and I did not have problems with it yet.
...
**** Xinput method
I prefer this method, since I can just add it to .xinitrc to persist it.
#+BEGIN_SRC bash :results output
...
Persisting this method requires a systemd service, so I prefer the previous method.
#+BEGIN_SRC bash :results output
...
[[https://bugzilla.redhat.com/show_bug.cgi?id=1200717#c2][Also see this post]].
