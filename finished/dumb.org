...
- Make the same query with plain ag,a if it was returned the bug is with dumb-nump regexps.
- If it failed the ag is removing results, if it failed make query with fallback ack.
- If ack failed make query with plain grep.
