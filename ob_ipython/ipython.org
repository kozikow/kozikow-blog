...
Although author of ob-ipython recommends ya-snippet, I prefer to only put very simple snippets into yasnippet.
I generate my code blocks with elisp. Added advantage is I automatically open it with =org-edit-src-code= afterwards.
My code for auto generating code blocks:
#+BEGIN_SRC clojure :results output
(defun my-get-unique-str ()
  "Adapted from http://ergoemacs.org/emacs/elisp_generate_uuid.html"
...
(defun my-org-insert-src-block (src-code-type)
  ...
  (interactive
   (let ((src-code-types
          '("python" "ipython-stdout" "ipython-plot" "scala" "java" "literal" "quote" "emacs-lisp" "C" "bash" "js" "cpp" "sql" "latex" "matlab" "org" "perl" "xml" "clojure")))
     (list (completing-read "Source code type: " src-code-types))))
  (progn
    (cond
     ((equal src-code-type "literal")
      (insert "#+BEGIN_EXAMPLE\n"))
     ((equal src-code-type "quote")
      (insert "#+BEGIN_QUOTE\n"))
     ((equal src-code-type "ipython-stdout")
      (insert "#+BEGIN_SRC ipython :session :results output :exports both\n"))
     ((equal src-code-type "ipython-plot")
      (insert (format "#+BEGIN_SRC ipython :session :file %s.png :exports both\n" (my-get-unique-str))))
     (t
